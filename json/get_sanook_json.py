...
# coding:utf-8
respostGet = requests.get("https://season.sanook.com/topoftheyear/")
...
groupArr = [{
    ...
    "item_id": [ 71 ],
    "item_score": 0,
    "category_name": "ศิลปินดาวรุ่งแห่งปี"
}]
...
for groupItem in groupArr:
    ...
    for voteItem in votes["choices"]:
        active_item = 2
        if groupItem['category_id'] == votes['id']:
            ...
            for item_vote_id in groupItem['item_id']:
                if voteItem['id'] == item_vote_id:
                    active_item = 1
            groupObj['list'].append({
                "item_name": voteItem['name'],
                "score": "/",
                "active_item": active_item,
                "weight": voteItem['percentage'],
                "item_image": "/"
            })
...
# 读取 json 文件
if not os.path.exists("vote-sanook.json"):
    with open('vote-sanook.json', 'w') as one:
        json_base = {"res": []}
        json.dump(json_base, one, ensure_ascii=False)

with open('vote-sanook.json', 'r') as fp:
...
with open('vote-sanook.json', 'w') as oldDate:
